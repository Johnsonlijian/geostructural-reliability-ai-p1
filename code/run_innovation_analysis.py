"""Tests the POSITIVE scientific thesis (before we claim it):
 A1 sufficiency : does full-feature ML beat the single mechanistic margin out-of-distribution?
 A2 screening   : inside the ambiguous band (FS~1), is the label still learnable from features?
 A3 irreducible : Bayes error vs mechanistic coordinate; is it localized at FS~1?
...
for D in load():
    ...
    # ---- A1 sufficiency: full features vs single mechanistic margin (out-of-distribution) ----
    ...
    # ---- A3 irreducible (Bayes) error vs mechanistic coordinate ----
...
for name, r in out.items():
    ...
    a1 = r["A1_sufficiency"]
    print(f" A1 SUFFICIENCY: physics_raw={a1['auc_physics_raw']}  margin-only-ML={a1['auc_margin_only_ML']}  full-feature-ML={a1['auc_full_feature_ML']}")
    ...
    a3 = r["A3_irreducible"]
    print(f" A3 IRREDUCIBLE: Bayes-error LB={a3['bayes_error_lower_bound']}  ML error={a3['ML_full_error']}  frac in ambiguous band={a3['frac_bayes_err_in_ambiguous_band']}")
